...
def _work(model, dataset, args):
    ...
    with torch.no_grad():
        ...
        for iter, pack in enumerate(tqdm(data_loader)):
            ...
            # img[0] shape : (2, 3, H, W)
            outputs = [model(img[0].cuda(non_blocking=True), label)
            ...
            # highres_cam = np.power(highres_cam, 0.8)
            ...
            # cam_img = cv2.applyColorMap(cam_img.astype(np.uint8), cmapy.cmap("RdYlGn_r"))
            image = cv2.addWeighted(image, 0.5, cam_img, 0.5, 0)
            
            # print(label)
            # print(label.shape) # 20 
            nonzero_labels = torch.nonzero(label)
            labels = list(map(str, sorted([i[0].item() for i in nonzero_labels])))
            label_str = "_".join(labels)

            cv2.imwrite(os.path.join(args.cam_vis_out_dir, f"{label_str}_{img_name}.png"), image)
